    ...
    public class cls_Property
    {
        ...
        public DataTable GetDreamHouseForSlider(int DreamHouseId)
        {
            ...
            try
            {
                ...
                string str = "";
                if (DreamHouseId==null || DreamHouseId==0)
                {
                    str = "select top(1) * from tbl_DreamHouseImages";
                }
                else
                {
                    str = "select * from tbl_DreamHouseImages where DreamHouseId=" + DreamHouseId;
                }
        ...
        public DataTable GetDreamHouseDetail(int DreamHouseId)
        {
            ...
            try
            {
                ...
                if (conn.State == ConnectionState.Closed)
                {
                 ...
                 if (DreamHouseId == null || DreamHouseId == 0)
                 {
                     str = "select top(1) * from tbl_DreamHouse";
                 }
                 else
                 {
                     str = "select * from tbl_DreamHouse where Id=" + DreamHouseId;
                 }
        ...
        public int InsertLogo(string Title, string filename, string Description)
        {
            int result = 0;
            try
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "usp_InsertLogo";
                cmd.Parameters.AddWithValue("@Title", Title);
                cmd.Parameters.AddWithValue("@ImageUrl", filename);
                cmd.Parameters.AddWithValue("@Description", Description);
        ...
        public int InsertBanners(string name,string filename,int itemorder,int ProjectId)
        {
            int result = 0;
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "Insert into tblBanner(Name,FileName,ItemOrder,ProjectId) values(@Name,@FileName,@ItemOrder,@ProjectId)";
                cmd.Parameters.AddWithValue("@Name", name);
        ...
        public DataTable GetBannerbyProjectId(int id)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                string str = "select * from tblBanner where ProjectId=" + id;
                SqlDataAdapter adp = new SqlDataAdapter(str, conn);
                adp.Fill(dt);
                dt.TableName = "tblBanner";
            }
