    ...
    public class PropertyApiController : ApiController
    {
                   ...
                   // PropertyCount = Properties.Count();
                    ...
                    foreach (var Property in Properties)
                    {
                        if (CheckExpireDate(Property))
                        {
                            ...
                            var Propertyimages = _PropertyImageService.GetPropertyImages().Where(c => c.PropertyId == Property.PropertyId).OrderByDescending(c => c.PropertyId);
        ...
        public HttpResponseMessage GetAllAgents(string PropertyTypeStatus ,int parentId)
        {
            ...
            try
            {
                ...
                var Properties = _AgentService.GetAgents().Where(c => c.AgentStatusId == PropertyStatusId && c.ParentId==parentId).OrderByDescending(x => x.AgentId);
